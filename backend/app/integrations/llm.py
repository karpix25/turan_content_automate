...
class LLMClient:
        ...
        content: list[dict[str, Any]] = [
            {
                ...
                "text": (
                    ...
                    "Найди главный смысл и перепиши его как короткий, бьющий в боль заголовок для "
                    "5-секундного вертикального видео. Заголовок должен сразу давать конфликт, пользу, "
                    "страх ошибки или главную суть, без длинного объяснения.\n\n"
                    "Правила: русский язык, 3-7 слов, максимум 60 символов, без эмодзи, без кавычек, "
                    "без ссылок и CTA, без вводных слов, без мягких формулировок вроде «вот на что». "
                    "Не копируй дословно, если можно сделать острее. Верни только текст заголовка.\n\n"
    ...
    def generate_infographic_reels_card(
        ...
        *,
        ...
        content: list[dict[str, Any]] = [
            {
                ...
                "text": (
                    ...
                    "Сначала прочитай текст на изображении/кадре и отдельно прочитай caption/описание. "
                    "Определи, где находится основной смысл: на изображении, в описании или в обоих источниках. "
                    "Если на изображении есть полноценный список/таблица/тезис, бери его как главный контент, а описание используй только для уточнения. "
                    "Если изображение почти без смысла или там только обложка/лицо/короткий хук, бери главный контент из описания. "
                    "Если картинка и описание дополняют друг друга, собери один цельный смысл без дублей. "
                    "Удали рекламу, ссылки, упоминания людей/аккаунтов, просьбы подписаться/лайкнуть/перейти, промо и водяные знаки. "
                    "Не выдумывай факты и не оставляй чужой CTA. Перепиши выбранный главный смысл в деловом, резком, понятном стиле автора. "
                    "Итоговая карточка должна быть новой нашей картинкой по промту, а не копией исходника.\n\n"
                    "Верни строго JSON без markdown с полями:\n"
                    "{\n"
                    '  "content_source": "image|caption|both",\n'
                    ...
                    "}\n\n"
                    "В image_prompt обязательно включи уже переписанный title/items/final_thought как точный русский текст для карточки. "
                    "Не проси генератор анализировать исходник повторно: промт должен быть самодостаточным и содержать финальный текст карточки.\n\n"
                    "image_prompt должен описывать готовую русскую карточку в таком стиле:\n"
                    "- вертикальный кадр 9:16;\n"
                    "- однотонный теплый золотисто-песочный фон строго #EBC97C (как эталонный фон, без градиента и смены оттенка);\n"
        ...
        content_source = str(payload.get("content_source") or "").strip().lower()
        if content_source not in {"image", "caption", "both"}:
            content_source = "both"
        ...
        if not image_prompt:
            image_prompt = (
                ...
                "Фон строго #EBC97C, теплый золотисто-песочный, без градиента, большой off-white блок, нижнее CTA-окно, "
