...
from .carousel_layout import build_layout_instruction, build_slide_text_spec
...
def build_master_image_prompt(
    ...
    profile = get_design_profile(design_format)
    layout_instruction = build_layout_instruction(composition_contract, design_format)
    prompt = (
        ...
        "Контракт композиции — это невидимая техническая разметка, а не текст для изображения: "
        "никогда не печатай его поля, проценты, координаты, размеры, номера или служебные подписи. "
        "Если контракт описывает нумерованные маркеры, это относится только к анализу референса и должно быть заменено "
        "на маркер «•» для нового контента. "
        ...
        "Используй только явную структуру ЗАГОЛОВОК/ОПИСАНИЕ из спецификации слайда; "
        "если описание пустое, не заполняй его текстом из референса. "
...
def build_slide_prompts(
    ...
    for index, part in enumerate(parts, start=1):
        ...
        slide_text = build_slide_text_spec(part, composition_contract, design_format)
        ...
        prompts.append(
            ...
            "СПЕЦИФИКАЦИЯ ТЕКСТА СЛАЙДА:\n{text}\n"
            "Выведи указанные заголовок и описание на русском языке, не добавляй новых мыслей и не меняй факты. "
            "Сохрани заданные переносы строк и количество строк буквально. Текст внутри блоков можно аккуратно "
            "сократить до вместимости зоны, без потери смысла; нельзя добавлять строки или переносить блок. "
            ...
            "если текста много, сначала сократи формулировку, затем уменьши размер шрифта внутри той же зоны; "
