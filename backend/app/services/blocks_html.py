...
.inner { display: flex; flex-direction: column; height: 100%; min-width: 0; }
.fit { flex: 1; display: flex; flex-direction: column; justify-content: flex-start;
       padding-top: calc(28px * var(--fit)); padding-bottom: calc(36px * var(--fit));
       gap: calc(36px * var(--fit)); }
...
.author span { font-size: 24px; color: var(--muted); white-space: nowrap;
               overflow: hidden; text-overflow: ellipsis; }
.page { font-size: 24px; color: var(--muted); flex: none; margin-left: 20px; }

.kicker { align-self: flex-start; background: var(--accent-grad); color: var(--accent-ink);
          font-size: calc(22px * var(--fit)); font-weight: 800; letter-spacing: 0.14em;
          text-transform: uppercase; padding: 12px 24px; border-radius: 999px; }
.title { font-size: calc(56px * var(--fit)); line-height: 1.14; font-weight: 800;
         letter-spacing: -0.01em; text-wrap: balance; }
.body { font-size: calc(36px * var(--fit)); line-height: 1.48; color: var(--ink);
        text-wrap: pretty; }

.type-cover .fit { flex: 1; justify-content: center; gap: calc(34px * var(--fit)); }
.type-cover .title { font-size: calc(70px * var(--fit)); line-height: 1.08; }
.type-cover .subtitle { font-size: calc(32px * var(--fit)); line-height: 1.45; color: var(--muted); }
...
.type-cta .cta-logo { width: calc(185px * var(--fit)); height: calc(185px * var(--fit));
                      border-radius: 50%; object-fit: cover; }
.type-cta .cta-pill { background: var(--accent-grad); color: var(--accent-ink);
                      font-size: calc(54px * var(--fit)); font-weight: 800; line-height: 1.2;
                      padding: 34px 60px; border-radius: 999px; text-align: center;
...
.type-cta .handle { display: flex; align-items: center; gap: 16px; color: var(--ink);
                    font-size: calc(29px * var(--fit)); font-weight: 600; }

.list { display: flex; flex-direction: column; gap: calc(18px * var(--fit)); }
.list .item { display: flex; align-items: flex-start; gap: 20px; }
.list .badge { flex: none; width: calc(46px * var(--fit)); height: calc(46px * var(--fit));
               border-radius: 50%; background: var(--accent-grad); color: var(--accent-ink);
               font-size: calc(26px * var(--fit)); font-weight: 800;
               display: flex; align-items: center; justify-content: center; }
.list .item p { font-size: calc(34px * var(--fit)); line-height: 1.38; padding-top: 7px; text-wrap: pretty; }
...
.table th { background: var(--accent-grad); color: var(--accent-ink); text-align: left;
            font-size: calc(26px * var(--fit)); font-weight: 700;
...
.table td { font-size: calc(28px * var(--fit)); line-height: 1.35;
            padding: calc(18px * var(--fit)) calc(22px * var(--fit));
...
.compare .card .side { font-size: calc(28px * var(--fit)); font-weight: 800; margin-bottom: calc(16px * var(--fit)); }
...
.compare .card li { font-size: calc(27px * var(--fit)); line-height: 1.35;
...
.qa .q { font-size: calc(30px * var(--fit)); line-height: 1.35; font-weight: 800; }
.qa .q::before { content: "? "; color: var(--accent); font-weight: 800; }
.qa .a { font-size: calc(32px * var(--fit)); line-height: 1.45; color: var(--muted); }

.body p + p { margin-top: calc(26px * var(--fit)); }
.takeaway { border-top: 2px solid var(--accent); padding-top: calc(24px * var(--fit));
            font-size: calc(30px * var(--fit)); line-height: 1.4; color: var(--accent); }
.takeaway strong { display: block; font-size: calc(20px * var(--fit)); letter-spacing: .1em; margin-bottom: 10px; }
.slide { overflow-wrap: normal; word-break: normal; hyphens: none; }
.table { table-layout: fixed; }
.format-story .title { font-size: calc(64px * var(--fit)); }
.format-story .body { font-size: calc(44px * var(--fit)); }
.format-story .list .item p { font-size: calc(40px * var(--fit)); }
.format-story .qa .a { font-size: calc(38px * var(--fit)); }
.format-story .takeaway { font-size: calc(36px * var(--fit)); }
.format-story.type-cover .title { font-size: calc(78px * var(--fit)); }
...
.type-stat .value { font-size: calc(150px * var(--fit)); line-height: 1.05; font-weight: 800;
                    color: var(--accent); letter-spacing: -0.02em; text-wrap: balance;
                    text-shadow: 0 12px 48px rgba(216, 175, 95, 0.25); }
.type-stat .caption { font-size: calc(31px * var(--fit)); line-height: 1.45; color: var(--muted); }

.type-quote .mark { font-size: calc(120px * var(--fit)); line-height: 0.6; font-weight: 800;
                    color: var(--accent); }
.type-quote .qtext { font-size: calc(40px * var(--fit)); line-height: 1.4; font-weight: 600;
                     text-wrap: pretty; }
.type-quote .qauthor { font-size: calc(26px * var(--fit)); color: var(--muted); }
"""
...
window.fitSlides = function () {
  ...
  document.querySelectorAll('.slide').forEach(function (slide) {
    ...
    function spills() {
      return inner.scrollHeight > inner.clientHeight + 1 || inner.scrollWidth > inner.clientWidth + 1
        || Array.from(inner.querySelectorAll('*')).some(function (el) {
          return el.scrollWidth > el.clientWidth + 1 && getComputedStyle(el).display !== 'inline';
        });
    }
    while (spills() && fit > 0.82) {
...
def _copy(value: Any) -> str:
    """Nonbreaking typography only; no editorial rearrangement or word splitting."""
    text = str(value if value is not None else "")
    text = re.sub(r"\b(в|во|к|ко|с|со|у|о|об|на|по|из|от|до|за|не|ни|и|а|но) +", r"\1" + "\u00a0", text, flags=re.IGNORECASE)
    text = re.sub(r"(\d) +(?=[А-Яа-яЁё%₽])", r"\1" + "\u00a0", text)
    text = re.sub(r" +([—–]) +", lambda match: "\u00a0" + match[1] + " ", text)
    # Keep a short final word with its predecessor; this changes spacing only.
    tail = re.search(r"(\S+) (\S+)$", text)
    if tail and len(tail[2].strip(".,!?…:;»")) <= 8 and len(tail[1]) + len(tail[2]) <= 24:
        text = text[:tail.start()] + tail[1] + "\u00a0" + tail[2]
    return _esc(text)


def _title_text(slide: dict) -> str:
    if slide.get("title_lines"):
        return "<br>".join(_copy(line) for line in slide["title_lines"])
    return _copy(slide.get("title", ""))


def _title_block(slide: dict, class_name: str = "title") -> str:
    return f'<h2 class="{class_name}">{_title_text(slide)}</h2>' if slide.get("title") else ""
...
def _slide_body(slide: dict) -> str:
    ...
    if slide_type == "cover":
        parts = []
        if slide.get("kicker"):
            parts.append(f'<div class="kicker">{_copy(slide["kicker"])}</div>')
        parts.append(f'<h1 class="title">{_title_text(slide)}</h1>')
        if slide.get("subtitle"):
            parts.append(f'<p class="subtitle">{_copy(slide["subtitle"])}</p>')
        return "".join(parts)
    if slide_type == "text":
        paragraphs = slide.get("paragraphs") or [slide["body"]]
        body = "".join(f"<p>{_copy(paragraph)}</p>" for paragraph in paragraphs)
        return _title_block(slide) + f'<div class="body">{body}</div>'
    if slide_type in ("checklist", "steps"):
        rendered_items = []
        for index, item in enumerate(slide["items"]):
            numbered = re.match(r"^(\d+)[.)]\s+(.+)$", item)
            marker = str(index + 1) if slide_type == "steps" else "✓"
            visible = item
            if slide_type == "checklist" and numbered:
                marker, visible = numbered.groups()
            rendered_items.append(
                f'<div class="item"><span class="badge">{_esc(marker)}</span>'
                f'<p>{_copy(visible)}</p></div>'
            )
        items = "".join(rendered_items)
        return _title_block(slide) + f'<div class="list">{items}</div>'
    if slide_type == "table":
        head = "".join(f"<th>{_copy(column)}</th>" for column in slide["columns"])
        rows = "".join(
            "<tr>" + "".join(f"<td>{_copy(cell)}</td>" for cell in row) + "</tr>"
    ...
    if slide_type == "comparison":
        def card(kind: str, title: str, items: list) -> str:
            lis = "".join(f"<li>{_copy(item)}</li>" for item in items)
            return (f'<div class="card {kind}"><div class="side">{_copy(title)}</div>'
                    f"<ul>{lis}</ul></div>")
    ...
    if slide_type == "stat":
        parts = [_title_block(slide),
                 f'<div class="value">{_copy(slide["value"])}</div>',
                 f'<p class="caption">{_copy(slide["caption"])}</p>']
        if slide.get("body"):
            parts.append(f'<p class="body">{_copy(slide["body"])}</p>')
        return "".join(parts)
    if slide_type == "qa":
        pairs = "".join(
            f'<div class="pair"><p class="q">{_copy(pair["q"])}</p><p class="a">{_copy(pair["a"])}</p></div>'
            for pair in slide["pairs"]
        )
        return _title_block(slide) + f'<div class="qa">{pairs}</div>'
    if slide_type == "quote":
        author = f'<div class="qauthor">— {_copy(slide["author"])}</div>' if slide.get("author") else ""
        return (f'<div class="mark">«</div>'
                + f'<p class="qtext">{_copy(slide["text"])}</p>' + author)
    if slide_type == "cta":
        handle = f'<div class="handle"><span>{_copy(slide.get("author") or "")}</span></div>' if slide.get("author") else ""
        return (f'<div class="cta-chip">'
                f'<img class="cta-logo" src="{logo_data_uri()}" alt="">'
                + handle +
                f'<div class="cta-pill">{_copy(slide["cta"])}</div></div>')
...
def build_slide_html(
    ...
    *,
    ...
    css = (_BASE_CSS
           ...
           .replace("PADY", "180" if height / width > 1.6 else "80"))
    body = _slide_body(slide)
    if slide.get("kicker") and slide["type"] not in ("cover", "cta"):
        body = f'<div class="kicker">{_copy(slide["kicker"])}</div>' + body
    if slide.get("takeaway"):
        body += f'<aside class="takeaway"><strong>ГЛАВНОЕ</strong>{_copy(slide["takeaway"])}</aside>'
    ...
    return (
        ...
        f"<section class=\"slide format-{'story' if height / width > 1.6 else 'carousel'} type-{_esc(slide['type'])} variant-{'b' if index % 2 else 'a'}\" data-index=\"{index}\">"
        f'<div class="inner"><div class="fit">{body}</div>'
