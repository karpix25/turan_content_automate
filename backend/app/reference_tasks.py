...
from .services.carousel_pipeline import resolve_reference_paths
from .services.reference_sources import extract_reference_post, resolve_project_platform_accounts
...
def _build_master_text(user: models.User, posts: list[models.ReferencePost]) -> str:
    payload = [
        {"title": post.title, "body": post.body, "source_url": post.source_url}
        for post in posts
    ]
    generated = llm._complete(build_reference_rewrite_prompt(payload, user.author_style_profile), temperature=0.65)
    return (generated or fallback_reference_text(payload)).strip()


def _create_daily_draft(db, user: models.User, project_id: int, posts: list[models.ReferencePost]) -> bool:
    ...
    if not platform_accounts:
        return False
    carousel_ctas = {}
    settings = db.query(models.PostMyPostProjectSetting).filter(
        models.PostMyPostProjectSetting.user_id == user.id,
        models.PostMyPostProjectSetting.project_id == project_id,
    ).first()
    if settings:
        carousel_ctas = dict(settings.carousel_ctas or {})
    if any(not carousel_ctas.get(platform) for platform in platform_accounts):
        logger.warning("Skipping daily carousel: CTA is missing for project %s", project_id)
        return False
    ...
    draft = models.CarouselDraft(
        ...
        master_text=_build_master_text(user, posts),
...
def sync_reference_channels_task(user_id: int | None = None, project_id: int | None = None) -> int:
    ...
    try:
        ...
        for (owner_id, owner_project_id), posts in grouped.items():
            ...
            if user and _create_daily_draft(db, user, owner_project_id, top_posts):
                created += 1
        return created
    finally:
        db.close()
